...
export const AdminUsersModal = ({ isOpen, onClose }) => {
  ...
  const [activeSubTab, setActiveSubTab] = useState('pending'); // 'pending' | 'all' | 'creds' | 'create_admin'
  ...
  // Fresh Super Admin Creation State (Official Admin Only)
  ...
  const handleUpdateAdminCreds = (e) => {
    ...
    if (!adminName || !adminEmail || !adminPassword) {
      alert("Please fill in all fields for the Super Admin account.");
      return;
    }
  ...
  const handleCreateOfficialSuperAdmin = async (e) => {
    e.preventDefault();
    if (!newAdminName || !newAdminEmail || !newAdminPassword) {
      alert("Please fill in all fields for the new Super Admin.");
      return;
    }
    try {
      await registerUser(newAdminName, newAdminEmail, newAdminPassword, 'Super Admin');
      alert(`✓ Official Super Admin Account created for ${newAdminName} (${newAdminEmail}) directly in Firebase Auth & Firestore!`);
      setNewAdminName('');
      setNewAdminEmail('');
      setNewAdminPassword('');
    } catch (err) {
      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full shadow-2xl border border-slate-200 overflow-hidden my-8">
        ...
        <div className="bg-gradient-to-r from-indigo-900 via-blue-900 to-indigo-800 p-5 text-white flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-extrabold text-base">Super Admin Roster & Official Admin Portal</h3>
              <p className="text-xs text-indigo-100">Manage user approvals, create official Super Admins, or update credentials</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex border-b border-slate-200 bg-slate-50 px-4 pt-3 overflow-x-auto">
          <button
            ...
            className={`px-3.5 py-2 text-xs font-extrabold transition border-b-2 flex items-center space-x-1.5 flex-shrink-0 ${
          ...
          <button
            onClick={() => setActiveSubTab('create_admin')}
            className={`px-3.5 py-2 text-xs font-extrabold transition border-b-2 flex items-center space-x-1.5 flex-shrink-0 ${
              activeSubTab === 'create_admin'
                ? 'border-indigo-600 text-indigo-900 bg-white rounded-t-xl'
                : 'border-transparent text-slate-500 hover:text-slate-800'
            }`}
          >
          ...
          >
            ...
            <span>Full Roster ({registeredUsers.length})</span>
          </button>
        ...
        <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto text-xs font-semibold">
          ...
          {/* TAB 2: CREATE OFFICIAL SUPER ADMIN (Firebase Auth Integrated) */}
          {activeSubTab === 'create_admin' && (
            <form onSubmit={handleCreateOfficialSuperAdmin} className="space-y-4 max-w-md bg-indigo-50/60 p-5 rounded-2xl border border-indigo-200">
              <div className="flex items-center space-x-2 text-indigo-950 font-black">
                <Crown className="w-5 h-5 text-amber-600" />
                <span className="text-sm">Create New Official Super Admin Account</span>
              </div>
              <p className="text-xs text-slate-600 font-medium leading-relaxed">
                Super Admin privileges are granted only by existing Super Admins. Accounts created here are automatically saved directly into <strong>Firebase Authentication & Cloud Firestore</strong>.
              </p>
              ...
              <button
                type="submit"
                className="w-full py-3 bg-gradient-to-r from-indigo-600 via-blue-600 to-teal-500 text-white font-black text-xs rounded-xl shadow-md transition hover:from-indigo-700 hover:to-teal-600 flex items-center justify-center space-x-2"
              >
                <Crown className="w-4 h-4 text-amber-300" />
                <span>Create Official Super Admin</span>
              </button>
            </form>
          )}

          {/* TAB 3: FULL USER ROSTER (SUPER ADMIN, COLLECTORS, VISITORS) */}
          {activeSubTab === 'all' && (
            <div className="space-y-3">
              <h4 className="font-black text-slate-800 uppercase tracking-wider text-[11px]">Registered Accounts & Roles</h4>
          ...
          {/* TAB 4: UPDATE SUPER ADMIN CREDENTIALS */}
          {activeSubTab === 'creds' && (
            <form onSubmit={handleUpdateAdminCreds} className="space-y-4 max-w-md bg-indigo-50/50 p-4 rounded-2xl border border-indigo-200">
              <div className="flex items-center space-x-2 text-indigo-900 font-black">
                ...
                <span>Update Active Super Admin Credentials</span>
              </div>
              <p className="text-[11px] text-slate-600 font-medium">
                Updating your details here locks the platform to your new email & password.
              </p>
